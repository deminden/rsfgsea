...
async fn main() -> Result<()> {
    ...
    let batch_results =
        engine.compute_es_batch(&weights, &hits_u32, k as u32, n_total as u32, 1, 0)?;
    ...
    for _ in 0..n_batch {
        ...
        for &h in &subset {
            all_subsets.push(h as u32);
        }
    }

    let gpu_results = engine.compute_es_batch(
        &weights,
        &all_subsets,
        k as u32,
        n_total as u32,
    ...
    for (i, gpu_res) in gpu_results.iter().enumerate() {
        let start = i * k;
        let subset_usize: Vec<usize> = all_subsets[start..start + k]
            .iter()
            .map(|&h| h as usize)
            .collect();
        let (cpu_es_batch, _) =
