...
async fn main() -> Result<()> {
    ...
    let mut correlations: Vec<(String, f64)> = gene_names.into_iter().zip(gene_data.iter())
        .map(|(name, data)| (name, calculate_spearman(data, &phenotype_ranks)))
        .collect();
    
    correlations.sort_by(|a, b| b.1.abs().partial_cmp(&a.1.abs()).unwrap());
    let top_500_genes: Vec<String> = correlations.iter().take(500).map(|(n, _)| n.clone()).collect();
    ...
    let _cpu_res = run_gsea(&ranks, std::slice::from_ref(&pathway), n_perm, 42, 1, 5000, 1.0, ScoreType::Std, 1.0);
    ...
    let gene_to_idx: std::collections::HashMap<String, usize> = ranks.genes.iter().enumerate().map(|(i, g)| (g.clone(), i)).collect();
    let hits: Vec<usize> = pathway.genes.iter().filter_map(|g| gene_to_idx.get(g).copied()).collect();

    let start_gpu = Instant::now();
    let _gpu_res = engine.fgsea_simple_pathway_with_buffer(&scores_buffer, &hits, &abs_weights_f32, n_perm, 42, 0)?;
    ...
    println!("  P-value: {:.4e}, Log2err: {:.4}", ml_res.p_value, ml_res.log2err);
    ...
    println!("  Simple Speedup: {:.2}x", cpu_duration.as_secs_f64() / gpu_duration.as_secs_f64());
