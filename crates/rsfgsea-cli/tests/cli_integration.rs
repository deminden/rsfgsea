...
#[test]
fn cli_gpu_rejects_simple_only_nperm_override() {
    let (_dir, ranks, gmt, output) = write_test_inputs();
    ...
    cli_bin()
        ...
        .failure()
        .stderr(predicate::str::contains(
            "--gpu does not support --nperm/simple-only forcing yet.",
        ));
}

#[test]
fn cli_gpu_rejects_custom_sample_size() {
    let (_dir, ranks, gmt, output) = write_test_inputs();
    ...
    cli_bin()
        ...
        .failure()
        .stderr(predicate::str::contains(
            "custom --sampleSize is not supported",
        ));
}
